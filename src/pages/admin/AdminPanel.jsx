    ...
    return (
        <div className="space-y-5">
            ...
            <div className="grid md:grid-cols-2 gap-4">
                ...
                <div className="md:col-span-2">{Field({ label: 'LinkedIn Headline (Sandbox)', field: 'linkedinTitle' })}</div>
            ...
            {Field({ label: 'About You (Homepage)', field: 'aboutStory', multiline: true })}
            {Field({ label: 'LinkedIn About (Sandbox)', field: 'linkedinAbout', multiline: true })}
            <hr className="border-neutral-100" />
    ...
    const handleImport = () => {
        try {
            ...
            if (parsed.profile) {
                ...
                if (parsed.profile.linkedinTitle) profileUpdates.linkedinTitle = parsed.profile.linkedinTitle;
                if (parsed.profile.title) {
                    profileUpdates.title = parsed.profile.title;
                    if (!parsed.profile.linkedinTitle) {
                        profileUpdates.linkedinTitle = parsed.profile.title;
                    }
                }
                if (parsed.profile.linkedinAbout) profileUpdates.linkedinAbout = parsed.profile.linkedinAbout;
                if (parsed.profile.aboutStory) {
                    // If it is regular import, we map aboutStory to homepage bio
                    profileUpdates.aboutStory = parsed.profile.aboutStory;
                    // If linkedinAbout isn't defined but aboutStory is, fallback map it to linkedinAbout
                    if (!parsed.profile.linkedinAbout) {
                        profileUpdates.linkedinAbout = parsed.profile.aboutStory;
                    }
                }
                const keepLocalImage = profile?.characterImage?.startsWith('/') && parsed.profile.characterImage && !parsed.profile.characterImage.startsWith('/');
                if (parsed.profile.characterImage && !keepLocalImage) {
                    profileUpdates.characterImage = parsed.profile.characterImage;
                }
